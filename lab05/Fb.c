...
bool FbUnfriend(Fb fb, char *name1, char *name2) {
    ...
    int id2 = nameToId(fb, name2);
    assert(id1 != id2);
    // if they're already not friends, return false
    ...
    if (inAdjList(fb->adj[id1], id2) == false) { 
        return true;
    }
    return false;
    // freeAdjList(AdjList l)
    // return false;
}
...
List FbMutualFriends(Fb fb, char *name1, char *name2) {
    ...
    while (curr1 != NULL) { 
        ...
        while (curr2 != NULL) { 
           // if (curr1->v == curr2->v)
          // printf("curr1 = %d, curr2 = %d", curr1->v, curr2->v);
            if (curr1->v == curr2->v) { 
                ListAppend(l, fb->names[curr1->v]);
                //printf("reached listappend");
            }
            curr2 = curr2->next;
...
int FbFriendRecs1(Fb fb, char *name, struct recommendation recs[]) {
    ...
    // List l = ListNew();
    AdjList curr1 = fb->adj[id];
    int second_key = curr1->v;
    AdjList curr2 = fb->adj[second_key];

    int *counts = calloc(fb->numPeople,  sizeof(int));
    while (curr1 != NULL) { 
        second_key = curr1->v;
        curr2 = fb->adj[second_key];
        while (curr2 != NULL) { 
           // if (curr1->v == curr2->v)
            printf("curr1 = %d, curr2 = %d\n", curr1->v, curr2->v);
            if (curr1->v != curr2->v && curr1->v != id) { 
                counts[curr2->v]++;
                printf("reached listappend\n");
            }
            curr2 = curr2->next;
