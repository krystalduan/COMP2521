...
// static bool testTreeInsertSameNode(void) { 
//     int vals[SIZE] = {10, 15, 25, 20, 23, 5, 1, 3, 22, 21, 17};
//     int absent[3] = {2, 30, -2};
//     Tree t = TreeNew();
...
static bool testTreeDeleteRotated(void) {
    printf(YLW "%s...\n" CLR, __func__);
    Tree t = makeTree();
    int vals[SIZE] = {15, 25, 22, 5, 23, 1, 17, 10, 20, 21, 3};
...
// my own test to check rotation
// static void testTreeInsertandDeleteRotatedHeight() { 
//     int vals[7] =  {4,3,2,6,7,9,5};
//     int del[3]= {3,5,7};
//     Tree t = TreeNew();
//     for (int i = 0; i < 7; i++) {
//         TreeInsert(t, vals[i]); 
...
// static bool testInsertDeleteLCA(void) {
//     printf(YLW "%s...\n" CLR, __func__);
//     Tree t = makeTree();
//     int aVals[5] = {1, 3, 20, 23, 17};
//     int bVals[5] = {5, 21, 17, 22, 25};
//     int vals[3] = {10, 23, 21};
    
//     int results_delete[5] = {3,15, 20, 23, 21};
//     TreeDelete(t, vals[0]);
//     // test 1
//     for (int i = 0; i < 5; i += 2) {
//        // TreeDelete(t, vals[i])/
//         int result_delete = TreeLCA(t, aVals[i], bVals[i]);
//         if (result_delete != results_delete[i])
//             PERR("1st Expected %d but got %d for LCA(%d, %d).\n", results_delete[i], result_delete, aVals[i], bVals[i]);

//     }
//     // test 2
//     // TreeDelete(t, vals[1]);
//     TreeDelete(t, vals[2]);
//     int results_delete_test2[5] = {3, UNDEFINED,20, 23,20};
//     for (int i = 0; i < 5; i += 1) {
//        // TreeDelete(t, vals[i])/
//         int result_delete = TreeLCA(t, aVals[i], bVals[i]);
//         printf("%d\n", result_delete);
//         if (result_delete != results_delete_test2[i])
//             PERR("Expected %d but got %d for LCA(%d, %d).\n", results_delete_test2[i], result_delete, aVals[i], bVals[i]);
...
static bool testTreeCeilingDelete(void) {
    printf(YLW "%s...\n" CLR, __func__);
    Tree t = makeTree();
    int vals[7] = {10, 2, 15, 18, -2, 22, 30};
    //int results[7] = {10, 3, 15, 20, 1, 23, UNDEFINED};
    
    // for (int i = 0; i < 7; i++) {
    //     int result = TreeCeiling(t, vals[i]);
    //     if (result != results[i])
    //         PERR("1stExpected %d but got %d as ceiling of %d.\n", results[i], result, vals[i]);
    // }
    TreeDelete(t, 10);
    TreeDelete(t, 20);
    TreeDelete(t, 17);
    TreeDelete(t, 15);
    int results2[7] = {21, 3, 21, 21, 1, 22, UNDEFINED};
    for (int i = 0; i < 7; i++) {
        int result = TreeCeiling(t, vals[i]);
        if (result != results2[i])
            PERR("Expected %d but got %d as ceiling of %d.\n", results2[i], result, vals[i]);
    }
    TreeFree(t);
    return true;
}
...
int main(int argc, char **argv) {
    bool (*tests[])(void) = {
       ...
       //testInsertDeleteLCA,
        //testTreeInsertandDeleteRotatedHeight,
        testTreeCeilingDelete,
        testTreeInsertSearch,
        testTreeDelete,
        testTreeDeleteRotated,
        testTreeToList,
        testTreeKthSmallest,
        testTreeKthLargest,
        testTreeLCA,
